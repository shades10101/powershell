# This powershell script was used to Create the Windows Server 2019 Image for mixp04 and mixp05.
# Only thing done manually was installing Xen-tools
...
# Activate Windows Server 2019 Standard
...
#Write-Host ($writeEmptyLine + "# Installing Windows updates, this may take awhile" + $writeSeperator + $time)`
...
#################################################################################################################################
# This will Install all Windows updates. 
#
#
#Install modules & packages
$nugetinstall = invoke-command -ScriptBlock {Install-PackageProvider -Name NuGet -MinimumVersion 2.8.5.201 -Force}
invoke-command -ScriptBlock {install-module pswindowsupdate -force}
invoke-command -scriptBlock {Import-Module PSWindowsUpdate -force}
...
#counts how many updates areavailable
